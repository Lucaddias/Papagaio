    ...
    var body: some View {
        LazyVStack(alignment: .leading, spacing: PapagaioTema.Espaco.curto) {
            ForEach(notas) { nota in
                ...
                .buttonStyle(.plain)
                .accessibilityHint(
                    "Inicia a reprodução a partir de \(nota.start.faladoPorExtenso)."
                )
            }
        }
    }
...
private struct LinhaDaNotaDaConversa: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(nota.start.faladoPorExtenso). \(ehMarcador ? "Marcador" : "Nota"). \(nota.texto)\(nota.critica ? ". Crítica." : "")")
    }
}
